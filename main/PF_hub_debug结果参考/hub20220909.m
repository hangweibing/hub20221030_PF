...
for i=1:N   %粒子集初始化
    ...
    %     logCstar=paraSet(1);
    %     gamma=paraSet(2);
    logCstar=unifrnd(-11.2,-10.6,1,1);
    gamma=normrnd(3,0.05,1,1);
    a=normrnd(2,0.05,1,1);
    c=a;
    for j=1:n_nodes
...
while (m-1)*step/1950.70866<=t_check(end)
    ...
    for i=1:N
        ...
        % 220914 做一个while循环，当出现发散的裂纹后，将其任意替换为一个正常的裂纹
        m_index=0;
        while m_index==0
...
%         axis equal
        ...
        xparticle(i,:,m)=[yRegSet,zRegSet,logCstar+normrnd(0,0.01,1,1),gamma+normrnd(0,0.01,1,1)];
        xparticle=real(xparticle);
...
% xparticle1(:,:,117)=xparticle(outindex,:,117);                                 %重采样
% for i=1:44
%     xparticle(:,i,117)=xparticle1(:,i,117)+h*D(i,117)*e(:,i,117);              %正则化
%     xparticle(:,i,117)=rearrange(xparticle(:,i,117),xparticle1(:,i,117))';     %参见文献《Dynamic Bayesian Network for Aircraft Wing Health Monitoring Digital Twin》
% end
% for i=1:N
...
% xparticle1(:,:,197)=xparticle(outindex,:,197);                             %重采样
% for i=1:44
%     xparticle(:,i,197)=xparticle1(:,i,197)+h*D(i,197)*e(:,i,197);          %正则化
%     xparticle(:,i,197)=rearrange(xparticle(:,i,197),xparticle1(:,i,197))'; %参见文献《Dynamic Bayesian Network for Aircraft Wing Health Monitoring Digital Twin》
% end
% for i=1:N
