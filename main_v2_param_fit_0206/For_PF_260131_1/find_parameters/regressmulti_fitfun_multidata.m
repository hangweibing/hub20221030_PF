...
if loss_mode>=2
    [eq,diff_omega,diff_omega_OF_deq,diff_delta_K, contains_k1,contains_k2]=diff_F(evalstr_in,num_const,diff_model);
...
%% 计算模型对于所有合并数据的系数
PARA0=Fitfun_multidata_merged(evalstr_in,gp,parameter_gp);
parameter_gp.PARA0=PARA0;
%% 第一层循环
...
%% 第二层循环    
    for data_i=1:Ndata                                             %第二层循环
        ...
        %parfor Kth_KC_i=1:N_ini_k1k2
            ...
            check_fitness=1;
            parameter_K=Parameters_meterial(Kth_KC_i,:);                        %作为非线性优化不同的起点
...
%             xtrain=[delta_kth./multidata{1,data_i}(:,1),multidata{1,data_i}(:,1)./delta_kth,multidata{1,data_i}(:,4)./kc,kc./multidata{1,data_i}(:,4),multidata{1,data_i}(:,3)];
            ...
            if check_fitness==1
                ...
                geneOutputs{Kth_KC_i,1}(:,numGenes+2)=lg(multidata{1,data_i}(:,1));                 %%最后补一列lg(delta_k)
                ...
                if loss_cal_optimize_in==1  
                    ...
                    showfigure=0;
                    %[test_index,~,~,~] = Multi_R_check(p_temp,showfigure,Const_pair_now,theta,delta_kth,kc);
                    [loss,delta_kth,kc,theta,opti_mode_used,constraint_wrong]=loss_cal_optimize(2.34,p_temp,DELTA_KTH,KC);
        ...
        show_figure=1;
        if show_figure
            para_data_i=fitness_at_Kth_KC_all{pvals,1}(2:end-1);
            theta=para_data_i(2+num_const+1:end);
            delta_kth=para_data_i(1);
            kc=para_data_i(2);
            [~,~,~,~] = Multi_R_check(p_temp,show_figure,Const_pair_now,theta,delta_kth,kc);

            saveFolder1 = 'C:\Users\wzy59\Desktop\test250526\Figure_NASGRO_HS_test';
            ...
            saveFolder2 = 'C:\Users\wzy59\Desktop\test250526\Parameters_NASGRO_HS_test';
...
if gp.fitness.gen_count_now<=gp.runcontrol.stage1        
    [~,pvals]=min(fitness_all_const{1,2});       %这里是备选方程在所有给定C参数范围内最优的loss_multidata作为fitness_out
end
...
if gp.fitness.gen_count_now>gp.runcontrol.stage2 
    [~,pvals]=min(fitness_all_const{1,4});       %pvals代表最优的一组C参数，当C只取值1的时候，就只有一组参数，没有影响
end
