function [test_index,output_struct] = Multi_R_check(p_temp,showfigure,Const_pair_now,theta,delta_kth,kc)
    ...
    NR = p_temp.NR;
    R_temp=p_temp.R_temp;
    Rg=p_temp.Rg;
    x1_temp_g=p_temp.x1_temp_g;
    KMg=p_temp.KMg;
    ...
    for i = 1:NR
        ...
        if numel(idx)>=3
            ...
            y_full  = y_grid(idx,i);
            dk = gradient(dK_full);        % 各点的 ΔdK
            dy = gradient(y_full);         % 各点的 Δy      
            % 变化率  gy_full = dy/ddK
            gy_full = dy ./ dk * c_log10 .* dK_full;
            R_val   = R_temp(i);
            ...
            % 1) 先按比例算出 p, 即头尾要取多少个点，最少取 1
            p = max( ceil(total * pct), 1 );              
            % 2) 再限制 p 最多只能到 floor((total-1)/2)，两侧加起来不超过 total-1，以保证中间至少留 1 点
            p = min( p, floor((total-1)/2) );
            % 3) 划分索引
            split1 = 1 : p;                  % Region I
            split3 = (total-p+1) : total;    % Region III
            split2 = (p+1) : (total-p);      % Region II
    ...
    N_mean_selected=floor(ptr2*0.2);
    % 检查长度是否足够
    a=output_struct.GY_2;
    a_sorted = sort(a);              % 升序排序
    a_trimmed = a_sorted(1:N_mean_selected);      % 选取区域II中斜率最小的前30%
    output_struct.GY_2_mean = mean(a_trimmed);    % 计算平均值
