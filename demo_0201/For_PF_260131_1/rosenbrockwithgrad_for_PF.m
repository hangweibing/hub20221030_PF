function [fit] = rosenbrockwithgrad_for_PF(z, p)
...
% --- 应用固定的具体数值 ---
if isfield(p, 'fix_k1') && p.fix_k1
    k(1) = p.k1_fixed_val; % 强制设为手动输入的固定值
end
if isfield(p, 'fix_k2') && p.fix_k2
    k(2) = p.k2_fixed_val; % 强制设为手动输入的固定值
end
if isfield(p, 'fix_theta3') && p.fix_theta3
    f(2) = p.theta3_fixed_val; % 强制设为手动输入的固定值
end
% -------------------------
if isfield(p, 'contains_k1') && isfield(p, 'contains_k2') && p.contains_k1 && p.contains_k2
    if isfield(p, 'k1k2_Correlation') && p.k1k2_Correlation
        k(1)=k0(1);          %k1不更新
    end
end
