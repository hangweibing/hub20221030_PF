function [mse, a_pred, da_history] = pred_a_N(p, t_check, z)
...
while N_now < N_total_end
    ...
    % 计算增量
    delta_a_step_N = sum(p.eq_fun(dk_m, Const_pair_now, f, k_param, kmax_m)) * 1e3;
    da_history(end+1) = delta_a_step_N;

    %% (5) 更新裂纹长度并推进时钟
    a_now = a_now + delta_a_step_N;
