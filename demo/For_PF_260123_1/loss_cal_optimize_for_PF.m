...
try
    ...
    [~, a_pred_last] = pred_a_N(p,omega(1, 1:3),omega(1, 4:5),Const_pair_now);
    figure('Color', 'w');
    ...
    hold on;
    plot(t_check, a_pred_last, 'b-o', 'LineWidth', 1.5, 'DisplayName', 'Predicted Trajectory');
    ...
    try
        % 这里是可能产生错误的代码
        p.fmincon_option2.Display = 'iter';
        [z_opt, fval, exitflag, output] = fmincon( ...
            fun,       ... % 目标函数
            z0,        ... % 初始点
            [], [],    ... % 线性不等式 A, b
