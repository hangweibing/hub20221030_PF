...
if isfield(p, 'fix_theta3') && p.fix_theta3
    f_actual(2) = p.theta3_fixed_val;
end
omega0 = [f_actual(:)', k_actual(:)'];
% -----------------------
...
% 固定参数设置
if isfield(p, 'fix_theta3') && p.fix_theta3
    % 固定 f2 (Index 2) 和 k1 (Index 4), 优化 f1, f3, k2 (1, 3, 5)
    z0_f  = z0([1, 3, 5]);
    LB_f  = LB([1, 3, 5]);
    UB_f  = UB([1, 3, 5]);
    fun_f = @(z_red) rosenbrockwithgrad_for_PF([z_red(1), 1, z_red(2), 1, z_red(3)], p);
elseif isfield(p, 'fix_k2') && p.fix_k2
...
try
    ...
    if isfield(p, 'fix_theta3') && p.fix_theta3
        z_opt = [z_opt_f(1), 1, z_opt_f(2), 1, z_opt_f(3)];
    elseif isfield(p, 'fix_k2') && p.fix_k2
