function [fit,G] = call_for_fminunc_3_2(z)
%提取全局变量
    ...
    %[~,delta_K_test,g_y,R] = Multi_R_check_old(global_p_temp,showfigure,Const_pair_now,theta,delta_kth,kc);
    ...
    if nargout > 1 % gradient required
        ...
        for i=1:num_p
            ...
            dM_dki=eval(diff_omega{i}).*omega0(i); 
            ddM_dki=eval(diff_omega_OF_deq{i})*log(10).*delta_K_test.*omega0(i);
            G1=1/numData*sum( (y_pre-ytrain).*dM_dki );
            G2=-1/N_gy*g_y_lambda1*sum( rou_g_y./(1+exp(g_y*g_y_alpha)).*ddM_dki );
