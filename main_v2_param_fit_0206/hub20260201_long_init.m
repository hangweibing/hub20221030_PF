...
% 如果知道真实参数，设置为 [-9.7360, 2.6089, 0.1603, 0.3940]
TRUE_PARAMS = []; 

% 安全地提取真实参数（如果存在）
if ~isempty(TRUE_PARAMS) && length(TRUE_PARAMS) >= 4
    true_log_theta1 = TRUE_PARAMS(1);
    true_theta2 = TRUE_PARAMS(2);
    true_theta3 = TRUE_PARAMS(3);
    true_k2 = TRUE_PARAMS(4);
else
    % 如果真实参数未知，设置为 NaN（不影响程序运行，仅影响可视化对比）
    true_log_theta1 = NaN;
    true_theta2 = NaN;
    true_theta3 = NaN;
    true_k2 = NaN;
    fprintf('[提示] 真实参数未设置，可视化时将不显示真实值对比线\n');
end
...
% 参数拟合模式配置
fit_fix_mode = 'k2';                    % 'k2': 固定 k2, 优化 theta; 'theta3': 固定 theta3, 优化 k2; 'none': 都不固定
k2_fixed_val = 0.2;                       % 当 fit_fix_mode='k2' 时生效
...
% 真实试验观测数据从大约13mm开始，裂纹已进入贯穿扩展阶段
% 初始裂纹长度均值12.8mm，对应z坐标均值42.8mm (30 + 12.8)
z_init_mean = 42.8;
z_init_std = 0.05;
z_min_limit = 37.9;

% y坐标在11和9之间均匀分布（21个点）
yIniRegSet = linspace(11, 9, n_nodes); 
zIniRegSet = zeros(1, n_nodes);          % 初始z坐标集占位
...
for i = 1:N   % 遍历所有粒子
    ...
    %% 几何参数采样（裂纹尺寸 - 贯穿阶段初始化）
    % 采样初始z坐标 (对应裂纹长度)
    z_val = normrnd(z_init_mean, z_init_std);
    
    % 确保z不小于最小值限制
    if z_val < z_min_limit
        z_val = z_min_limit;
    end
    
    % 该粒子的所有21个节点z坐标相同
    zIniRegSet = repmat(z_val, 1, n_nodes);
...
if any(valid_mask_init)
    ...
    % 记录参数的置信区间
    param_upper(:, 1) = prctile(xparticle_curr(valid_mask_init, 43:46), 95, 1)';
    param_lower(:, 1) = prctile(xparticle_curr(valid_mask_init, 43:46), 5, 1)';
end
% -------------------------------------
...
while (m-1)*step/cyclesperhour <= t_check(end)
    ...
    if any(valid_mask)
        ...
        % 记录参数的 90% 置信区间
        param_upper(:, m) = prctile(xparticle_curr(valid_mask, 43:46), 95, 1)';
        param_lower(:, m) = prctile(xparticle_curr(valid_mask, 43:46), 5, 1)';
    ...
    if ((m-1)*step/cyclesperhour < t_check(j)) && (m*step/cyclesperhour >= t_check(j))
        ...
        if j > 1
            ...
            % 执行多起始点优化 (传入 prev_PARA0_fit 以利用历史拟合结果作为热启动起始点)
            N_starts_opt = 10;
            [~, delta_kth_fit, kc_fit, theta_fit, ~, ~, pass_idx] = ...
                loss_cal_optimize_for_PF_multistart(parameter_gp, pop_now, N_starts_opt, prev_PARA0_fit);
        ...
        xpf_params = [Xpf(43, 1:m); Xpf(44, 1:m); Xpf(45, 1:m); Xpf(46, 1:m)];
        plot_parameter_evolution(xpf_params, param_upper(:, 1:m), param_lower(:, 1:m), ...
            (0:m-1)*step/cyclesperhour, j, TRUE_PARAMS);

        % --- [Multi-Weight] 4. 联合权重重采样 (改进版) ---
        % 配置重采样选项
        resample_opts = struct();
        resample_opts.weight_mode = 'min_normalized';      % 使用最小归一化模式组合各维度权重
    ...
    %     buffer_log_theta3_particles(:, buffer_step_counter) = xparticle_curr(:, 45);
...
%% ===================================================================
%% 8. 输出参数拟合历史结果汇总 (利用 prev_PARA0_fit)
%% ===================================================================
if ~isempty(prev_PARA0_fit)
    fprintf('\n--- 参数拟合历史结果汇总 ---\n');
    fprintf('%-10s %-12s %-12s %-10s %-10s %-10s\n', '观测步', 'delta_Kth', 'Kc', 'f1(log_th1)', 'f2(th3)', 'f3(th2)');
    for row = 1:size(prev_PARA0_fit, 1)
        % 注意：j 是从 2 开始拟合的（第一次拟合通常在第 2 次观测时）
        obs_idx = row + 1; 
    ...
    % 将历史结果导出为 Table 方便在变量浏览器查看
    para_fit_history = array2table(prev_PARA0_fit, ...
        'VariableNames', {'delta_Kth', 'Kc', 'f1', 'f2', 'f3'});
    assignin('base', 'para_fit_history_long', para_fit_history);
else
    fprintf('\n在此次运行中未产生成功的参数拟合结果。\n');
end
fprintf('程序运行总耗时: %.2f 秒\n', toc(total_tic));
