...
%
% 输入参数：
%   xparticle     - [N x D] 粒子矩阵，每行一个粒子 [a, log_theta1, theta2, theta3, k2]
%   multi_weights - [N x D] 多维度权重矩阵，每列对应一个状态维度的似然权重
%   options       - (可选) 结构体，包含以下字段：
%       .weight_mode   - 权重组合模式: 
%                        'product' (默认): 各维度权重相乘
%                        'weighted_sum': 各维度权重加权平均
%                        'min_normalized': 各维度归一化后取最小值
%       .dim_weights   - 各维度的重要性权重 [1 x D]，仅在 weighted_sum 模式下使用
...
dim_weights = get_option(options, 'dim_weights', ones(1, D));
...
switch weight_mode
    ...
    case 'min_normalized'
        % 归一化各维度的似然权重
        % norm_multi_weights = multi_weights ./ (sum(multi_weights, 1) + eps);
        norm_multi_weights = multi_weights;
        % 取各维度中的最小值作为联合权重
        joint_weights = min(norm_multi_weights, [], 2);
...
% 使用 Silverman's rule of thumb 估计带宽
...
fprintf('  [联合重采样] 完成！应用核带宽: [%.4f, %.4f, %.4f, %.4f, %.4f]\n', bandwidth);
